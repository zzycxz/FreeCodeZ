...
import { CodingPlanStatusMeta } from "./CodingPlanStatusMeta.js";
...
export function CodingPlanStatusPanel({
  ...
  const canDisconnectProvider =
    ...
    !isUnavailable &&
    !isUnsupported;
  const loginButtonId = "settings.modelProvider.codingPlan.connect";
  ...
  const statusBadgeId = isChecking ? defaultStatusBadgeId : (statusLabelId ?? defaultStatusBadgeId);
  ...
  const usageCardsVisible =
    usageDetailsVisible && isPurchased && hasDisplayableCodingPlanUsageLimits(quotaLimits);
...
function resolveCodingPlanStatusCardTitle({
  ...
  if (!isPurchased) {
    return inactivePlanTitle?.trim() || codingPlanTitle;
  }
