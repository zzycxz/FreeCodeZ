...
// e2e 运行的是生产构建，默认会和本机正式版 FreeCodeZ 共用 app name / userData，
...
export const runtimeApplicationName =
  ...
  // FreeCodeZ fork:运行名与原版 ZCode 分离,单实例锁与 %APPDATA% 目录随之隔离(规格书 P1 §1.2)。
...
export function loadHostProcessEnvFromLocalFiles(): Record<string, string> {
  if (isElectronAppPackaged()) {
    // 安装包不内嵌运行时连接配置，避免 CI 凭据随产物公开。
    // 只保留打包身份元数据，缺少端点时不会启用上报。
    return {}; // FreeCodeZ fork(P3):遥测分布标记已删
  }
...
export function buildHostProcessEnv(hostProcessLocalEnv: Record<string, string>) {
      ...
      : packagedDesktop
        ...
        : // Truthy grammar must match the producer's isUnsignedHelperLocalDevRequested
          ...
          ? rawInheritedEnv.ZCODE_CUA_BUNDLED_HELPER_APP_PATH?.trim() ||
            join(
              rawInheritedEnv.ZCODE_HOME?.trim() || join(homedir(), ".freecodez"),
              "computer-use",
              "dev",
              DEV_HELPER_APP_NAME,
            )
  ...
  // Desktop 身份由 host 从凭据仓库和本机状态读取后可信注入；外部环境不能注入身份，
  ...
  return {
    ...inheritedEnv,
    // 出口凭据只定向传到 host；host 初始化 services 时会立即捕获并从 process.env 清除，
